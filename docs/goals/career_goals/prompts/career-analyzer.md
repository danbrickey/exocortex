...
### File Organization:
- **Current version**: Store in `docs/goals/career_goals/career_plan.md` (no version suffix)
- **Past versions**: Move to `docs/goals/career_goals/career_plan/career_plan_v[X.Y].md`
...
The user has implemented a **Personal Operating System** (see `docs/PERSONAL_OPERATING_SYSTEM.md`) that integrates:
- BCI-specific goals (`docs/goals/bci_goals/`)
- Career goals (`docs/goals/career_goals/`)
- Daily/weekly/monthly tracking (`docs/personal/`)
- Work tracking (`docs/work_tracking/`)
- Report generation (`docs/reports/`)

### When Working with Career Plans:

**Career Plan Location**: `docs/goals/career_goals/career_plan.md`

**Related Files**:
- BCI goals: `docs/goals/bci_goals/current_goals.md`, `vision.md`, `architecture_goals.md`
- Monthly reviews: `docs/personal/monthly_reviews/` - Track progress on career plan milestones
- Accomplishment log: `docs/reports/performance_reviews/accomplishment_log.md` - Portfolio evidence
- Learning tracker: `docs/goals/career_goals/learning_tracker.md` (if exists)

### Integration Points:

1. **Phase 0 (Workflow Optimization)** in career plan overlaps with BCI delegation goals
2. **Blue Cross AI transformation work** feeds both BCI goals AND career portfolio
3. **Monthly reviews** assess progress on BOTH goal systems simultaneously
4. **Accomplishment log** captures work for both performance reviews AND external portfolio

### When Updating Career Plan:

- Consider user's current BCI goals and time allocation
- Reference monthly reviews for actual progress vs. planned progress
- Update portfolio tracker with completed projects
- Ensure career plan timeline is realistic given BCI commitments

### Report Generation:

Career accomplishments can be pulled for:
- **BCI leadership reports**: Focus on business impact and team development
- **Performance reviews**: Focus on accomplishments and growth
- **Career portfolio**: Focus on technical projects and external positioning
- **LinkedIn updates**: Focus on thought leadership and professional brand
