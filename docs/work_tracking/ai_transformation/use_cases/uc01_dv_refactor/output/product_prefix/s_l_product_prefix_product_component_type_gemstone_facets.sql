{{
    config(
        ...
        tags=['raw_vault', 'satellite', 'link_satellite', 'product_prefix', 'gemstone_facets']
    )
}}

/*
    Link Satellite: product_prefix_product_component_type (Gemstone Facets)
    Parent Link: l_product_prefix_product_component_type
    Source System: gemstone_facets
    Source Table: dbo.cmc_pdpx_desc

    Purpose: Stores descriptive attributes for the product prefix and component
    type relationship from the gemstone_facets source system. Tracks changes over
...
{%- set yaml_metadata -%}
source_model: "stg_product_prefix_gemstone_facets"
