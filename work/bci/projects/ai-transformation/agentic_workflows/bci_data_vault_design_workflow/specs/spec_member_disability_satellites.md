...
As a data engineer,  
I want to create the member_disability satellites in the raw vault,  
...
- `stg_gemstone_facets_hist__dbo_cmc_mehd_handicap` - Member disability data from Gemstone system
- `stg_legacy_bcifacets_hist__dbo_cmc_mehd_handicap` - Member disability data from legacy system
...
- stg_member_disability_gemstone_facets_rename - Rename columns for gemstone facets
- stg_member_disability_legacy_facets_rename - Rename columns for legacy facets
...
source as (
    select
        ...
        mehd.mehd_desc,
        mehd.mehd_type,
        mehd.mehd_last_ver_dt,
        mehd.mehd_last_ver_name,
        mehd.mehd_mctr_vmth,
        mehd.mehd_lock_token,
        mehd.atxr_source_id,
        mehd.edp_start_dt,
        mehd.edp_record_status
...
- stg_member_disability_gemstone_facets - Stage data from cmc_mehd_handicap for gemstone facets
- stg_member_disability_legacy_facets - Stage data from cmc_mehd_handicap for legacy facets

**Satellites** (using automate_dv sat macro):

- s_member_disability_gemstone_facets - Descriptive attributes from Gemstone system
- s_member_disability_legacy_facets - Descriptive attributes from legacy system
...
**Passed:** 10 / 10
- ✅ **Title & Description**: Title includes Domain (Member) and Entity (member_disability). Description accurately reflects objects being built (satellites only).
...
**Passed:** 6 / 6
- ✅ **Join Logic Documentation**: Staging join example includes multiple tables (cmc_mehd_handicap, cmc_meme_member, cmc_sbsb_subsc) and example is provided and complete with join conditions.
...
- ✅ **Consistency**: Description objects match Technical Details objects (satellites). Entity name (member_disability) used consistently throughout.
...
- ✅ **Satellite vs Reference Table**: Satellites are appropriately used for descriptive attributes that change over time (disability types, effective dates, verification history) and need historization. This is not static lookup data.
...
- ✅ **Satellite Rate of Change**: Disability data has moderate rate of change (effective dates, verification updates) which is appropriate for standard satellites. No need for hroc/mroc/lroc split.
...
- ✅ **No Over-Engineering**: Appropriate use of satellites for time-varying disability attributes. Not over-engineered.
...
1. **Can an engineer identify all source models?** Yes - All source models are listed with full project and model names: `stg_gemstone_facets_hist__dbo_cmc_mehd_handicap`, `stg_legacy_bcifacets_hist__dbo_cmc_mehd_handicap`, `stg_gemstone_facets_hist__dbo_cmc_meme_member`, `stg_gemstone_facets_hist__dbo_cmc_sbsb_subsc`
