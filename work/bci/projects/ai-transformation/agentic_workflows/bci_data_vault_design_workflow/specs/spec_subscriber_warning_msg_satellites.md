## Member 360: Build Raw Vault subscriber_warning_msg Satellites
...
As a data engineer,  
I want to create the subscriber_warning_msg satellites in the raw vault,  
...
- `stg_gemstone_facets_hist__dbo_cmc_sbwm_sb_msg` - Subscriber warning message data from Gemstone system
- `stg_legacy_bcifacets_hist__dbo_cmc_sbwm_sb_msg` - Subscriber warning message data from legacy system
...
- stg_subscriber_warning_msg_gemstone_facets_rename - Rename columns for gemstone facets
- stg_subscriber_warning_msg_legacy_facets_rename - Rename columns for legacy facets
...
source as (
    select
        ...
        sbwm.sbwm_term_dt,
        sbwm.sbwm_mctr_trsn,
        sbwm.sbwm_lock_token,
        sbwm.atxr_source_id,
        sbwm.sys_last_upd_dtm,
        sbwm.sys_usus_id,
        sbwm.sys_dbuser_id,
        sbwm.edp_start_dt,
        sbwm.edp_record_status
...
- stg_subscriber_warning_msg_gemstone_facets - Stage data from cmc_sbwm_sb_msg for gemstone facets
- stg_subscriber_warning_msg_legacy_facets - Stage data from cmc_sbwm_sb_msg for legacy facets

**Satellites** (using automate_dv sat macro):

- s_subscriber_warning_msg_gemstone_facets - Descriptive attributes from Gemstone system
- s_subscriber_warning_msg_legacy_facets - Descriptive attributes from legacy system
...
**Passed:** 10 / 10
- ✅ **Title & Description**: Title includes Domain (Member) and Entity (subscriber_warning_msg). Description accurately reflects objects being built (satellites only).
...
**Passed:** 6 / 6
- ✅ **Join Logic Documentation**: Staging join example includes multiple tables (cmc_sbwm_sb_msg, cmc_sbsb_subsc, cmc_meme_member) and example is provided and complete with join conditions. Join logic corrected to use `sbsb.sbsb_ck = mem.sbsb_ck` to properly link subscriber to member.
...
- ✅ **Consistency**: Description objects match Technical Details objects (satellites). Entity name (subscriber_warning_msg) used consistently throughout.
...
- ✅ **Satellite vs Reference Table**: Satellites are appropriately used for descriptive attributes that change over time (warning messages, effective dates, termination reasons) and need historization. This is not static lookup data.
- ✅ **Link Appropriateness**: Not applicable - no links in this specification.
- ✅ **Business Key Granularity**: Business key represents the correct level of detail (member level, inherited from parent hub). Note: Warning message data is at subscriber level but attached to member hub for business context.
- ✅ **Satellite Rate of Change**: Warning message data has moderate rate of change (effective dates, termination dates, message updates) which is appropriate for standard satellites. No need for hroc/mroc/lroc split.
...
- ✅ **No Over-Engineering**: Appropriate use of satellites for time-varying warning message attributes. Not over-engineered.
...
1. **Can an engineer identify all source models?** Yes - All source models are listed with full project and model names: `stg_gemstone_facets_hist__dbo_cmc_sbwm_sb_msg`, `stg_legacy_bcifacets_hist__dbo_cmc_sbwm_sb_msg`, `stg_gemstone_facets_hist__dbo_cmc_sbsb_subsc`, `stg_gemstone_facets_hist__dbo_cmc_meme_member`
