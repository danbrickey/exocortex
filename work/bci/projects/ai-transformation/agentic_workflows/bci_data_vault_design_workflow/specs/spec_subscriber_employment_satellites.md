## Member 360: Build Raw Vault subscriber_employment Satellites
...
As a data engineer,  
I want to create the subscriber_employment satellites in the raw vault,  
...
- `stg_gemstone_facets_hist__dbo_cmc_sbem_employ` - Subscriber employment data from Gemstone system
- `stg_legacy_bcifacets_hist__dbo_cmc_sbem_employ` - Subscriber employment data from legacy system
...
- stg_subscriber_employment_gemstone_facets_rename - Rename columns for gemstone facets
- stg_subscriber_employment_legacy_facets_rename - Rename columns for legacy facets
...
source as (
    select
        ...
        sbem.sbem_type,
        sbem.sbem_mctr_dtyp,
        sbem.sbem_lock_token,
        sbem.atxr_source_id,
        sbem.edp_start_dt,
        sbem.edp_record_status
...
- stg_subscriber_employment_gemstone_facets - Stage data from cmc_sbem_employ for gemstone facets
- stg_subscriber_employment_legacy_facets - Stage data from cmc_sbem_employ for legacy facets

**Satellites** (using automate_dv sat macro):

- s_subscriber_employment_gemstone_facets - Descriptive attributes from Gemstone system
- s_subscriber_employment_legacy_facets - Descriptive attributes from legacy system
...
**Passed:** 10 / 10
- ✅ **Title & Description**: Title includes Domain (Member) and Entity (subscriber_employment). Description accurately reflects objects being built (satellites only).
...
**Passed:** 6 / 6
- ✅ **Join Logic Documentation**: Staging join example includes multiple tables (cmc_sbem_employ, cmc_sbsb_subsc, cmc_meme_member) and example is provided and complete with join conditions. Join logic corrected to use `sbsb.sbsb_ck = mem.sbsb_ck` to properly link subscriber to member.
...
- ✅ **Consistency**: Description objects match Technical Details objects (satellites). Entity name (subscriber_employment) used consistently throughout.
...
- ✅ **Satellite vs Reference Table**: Satellites are appropriately used for descriptive attributes that change over time (employment information, effective dates, occupation, department, location) and need historization. This is not static lookup data.
- ✅ **Link Appropriateness**: Not applicable - no links in this specification.
- ✅ **Business Key Granularity**: Business key represents the correct level of detail (member level, inherited from parent hub). Note: Employment data is at subscriber level but attached to member hub for business context.
- ✅ **Satellite Rate of Change**: Employment data has moderate rate of change (effective dates, occupation changes, department transfers) which is appropriate for standard satellites. No need for hroc/mroc/lroc split.
...
- ✅ **No Over-Engineering**: Appropriate use of satellites for time-varying employment attributes. Not over-engineered.
...
1. **Can an engineer identify all source models?** Yes - All source models are listed with full project and model names: `stg_gemstone_facets_hist__dbo_cmc_sbem_employ`, `stg_legacy_bcifacets_hist__dbo_cmc_sbem_employ`, `stg_gemstone_facets_hist__dbo_cmc_sbsb_subsc`, `stg_gemstone_facets_hist__dbo_cmc_meme_member`
