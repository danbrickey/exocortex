{{
    config(
        materialized='incremental',
        unique_key='benefit_summary_type_hk',
        tags=['raw_vault', 'hub']
    )
}}

{%- set source_models = [
    ref('stg_benefit_summary_legacy_facets'),
    ref('stg_benefit_summary_gemstone_facets')
] -%}

{%- set yaml_metadata -%}
source_model:
  stg_benefit_summary_legacy_facets:
    src_pk: benefit_summary_type_hk
    src_nk: benefit_summary_type_bk
    src_ldts: load_datetime
    src_source: record_source
  stg_benefit_summary_gemstone_facets:
    src_pk: benefit_summary_type_hk
    src_nk: benefit_summary_type_bk
    src_ldts: load_datetime
    src_source: record_source
{%- endset -%}

{% set metadata_dict = fromyaml(yaml_metadata) %}

with

{% for source_model in source_models %}
staging_{{ loop.index }} as (
    select
        benefit_summary_type_hk,
        benefit_summary_type_bk,
        load_datetime,
        record_source
    from {{ source_model }}
    where benefit_summary_type_bk is not null
),
{% endfor %}

staging_union as (
    {% for source_model in source_models %}
    select * from staging_{{ loop.index }}
    {% if not loop.last %}union all{% endif %}
    {% endfor %}
),

records_to_insert as (
    select distinct
        benefit_summary_type_hk,
        benefit_summary_type_bk,
        load_datetime,
        record_source
    from staging_union
    {% if is_incremental() %}
    where benefit_summary_type_hk not in (
        select benefit_summary_type_hk from {{ this }}
    )
    {% endif %}
)

select * from records_to_insert
