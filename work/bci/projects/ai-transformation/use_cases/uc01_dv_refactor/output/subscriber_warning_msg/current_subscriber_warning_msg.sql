...
{% for source_system in source_systems %}
  {% if source_system == "legacy_facets" %}
    {% set sat_model = "s_subscriber_warning_msg_legacy_facets" %}
  {% else %}
    {% set sat_model = "s_subscriber_warning_msg_gemstone_facets" %}
  {% endif %}
  ...
  select
    ...
    sat.warning_msg_eff_dt,
    sat.message_id,
    sat.warning_msg_term_dt,
  ...
  join (
    select subscriber_hk, warning_msg_eff_dt, max(load_datetime) as max_load_datetime
    from {{ ref(sat_model) }}
    group by subscriber_hk, warning_msg_eff_dt
  ) as latest
    on sat.subscriber_hk = latest.subscriber_hk
   and sat.warning_msg_eff_dt = latest.warning_msg_eff_dt
