...
{% for source_system in source_systems %}
  {% if source_system == "legacy_facets" %}
    {% set sat_model = "s_subscriber_employment_legacy_facets" %}
  {% else %}
    {% set sat_model = "s_subscriber_employment_gemstone_facets" %}
  {% endif %}
  ...
  select
    ...
    sat.employment_eff_dt,
    sat.employment_term_dt,
    sat.employment_term_reason_cd,
    sat.group_bk,
    sat.occupation_cd,
    sat.department_cd,
    sat.location_cd,
    sat.employment_type_cd,
    sat.non_discrimination_type_cd,
    sat.lock_token_nbr,
  ...
  join (
    select subscriber_hk, employment_eff_dt, max(load_datetime) as max_load_datetime
    from {{ ref(sat_model) }}
    group by subscriber_hk, employment_eff_dt
  ) as latest
    on sat.subscriber_hk = latest.subscriber_hk
   and sat.employment_eff_dt = latest.employment_eff_dt
