# Title: Raw vault data pipeline for member_student_status

## Description

As a data engineer,
I want to rewrite the existing dbt model for the member_student_status entity, which currently supports a 3NF data model,
...
The member_student_status entity captures student eligibility verification information including effective/termination dates, school information, verification details, and student type classifications. This data exists in both legacy_facets and gemstone_facets source systems and needs to be integrated into the Data Vault architecture as effectivity satellites attached directly to the existing h_member hub to properly track temporal changes in student status.

## Acceptance Criteria

Given the design document is provided,
When I create a feature branch in the edp_data_domains repository,
Then I name the branch `feature/member-student-status-dv2-refactor` and base it on the main branch.

Given the design document is provided,
When I implement the raw vault models,
Then I create the appropriate rename views, staging, and effectivity satellite dbt models for the member_student_status entity following the automate_dv package conventions and naming standards.
...
- Descriptions for all columns including:
  ...
  - student_eff_dt (effective date)
  - student_term_dt (termination date)
  - termination_reason_cd
  - school_name
  - student_type
  - verification details (last_verification_dt, last_verification_name, verification_method_cd)
  - group_bk
  - All other student status attributes
...
- Not null tests for student_eff_dt
...
- Data quality tests appropriate for the entity type:
  - Valid date ranges (student_eff_dt <= student_term_dt when both are not null)
...
- Filters to current/active records only (latest load_datetime per member_hk and student_eff_dt combination)
...
- Reference the source table (dbo.cmc_mest_student) being transformed
...
- Explain the effectivity satellite pattern for tracking temporal student status changes
- Tag appropriate reviewers for code review

## Technical Notes

- The member_student_status entity uses effectivity satellites attached directly to the existing h_member hub
- The satellites use member_hk as the src_pk, allowing multiple student status periods per member
- Effectivity satellites are used because student status data has temporal aspects (effective and termination dates)
- The src_eff, src_start_date, and src_end_date parameters in the effectivity satellites handle the temporal logic
- Special handling may be needed for open-ended records where student_term_dt indicates no termination
- Both source systems (legacy_facets and gemstone_facets) need separate staging and satellite models
- The current view unions data from both sources and filters to the latest version per member_hk and student_eff_dt combination

## Definition of Done

- [ ] Feature branch created: feature/member-student-status-dv2-refactor
